 ...
 * (número + swatch), para consistencia visual total.
 * Flujo de asignación (sin ciclos, sin ambigüedad):
 *  1) Click en "Asignar" de una técnica -> queda seleccionada (fila resaltada).
 *  2) Click en una posición -> se asigna AHÍ. Lo que hubiera en esa posición se reemplaza.
 *  Click en una posición ocupada SIN técnica seleccionada -> la desasigna.
 * La UI se actualiza EN TIEMPO REAL: el cambio se aplica optimista en el cliente
 * (misma manera bind() que ejecuta el servidor) y el sync del servidor lo confirma.
 */
public class KiTechniquesScreen extends ZenkaiMenuScreen {
    ...
    protected void initContent() {
        ...
        for (int i = 0; i < slots.size(); i++) {
            final int idx = i;
            this.addRenderableWidget(new TextOnlyButton(panelLeft + BG_W - 154, y, 46, 16,
                    Component.translatable("screen.zenkai.technique.assign"),
                    () -> {
                        assigning = (assigning == idx) ? -1 : idx; // toggle
                    }));
            this.addRenderableWidget(new TextOnlyButton(panelLeft + BG_W - 104, y, 44, 16,
                    Component.translatable("screen.zenkai.technique.edit"),
                    () -> mc.setScreen(new TechniqueEditScreen(idx))));
            this.addRenderableWidget(new TextOnlyButton(panelLeft + BG_W - 56, y, 16, 16,
                    Component.literal("✖"),
                    () -> {
    ...
    public void render(@NotNull GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        for (int pos = 0; pos < n; pos++) {
            ...
            if (t != null) {
                g.fill(bx + 3, by + 3, bx + CELL - 3, by + CELL - 3, 0xFF000000 | t.rgb());
            }
        ...
        for (int i = 0; i < slots.size(); i++) {
            ...
            if (sel) { // fila resaltada mientras se asigna
                g.fill(panelLeft + 12, y - 2, panelLeft + BG_W - 12, y + 20, 0x30FFFFFF);
            }
            g.fill(panelLeft + 16, y + 2, panelLeft + 28, y + 14, 0xFF000000);
            g.fill(panelLeft + 17, y + 3, panelLeft + 27, y + 13, 0xFF000000 | t.rgb());
            g.drawString(this.font, Component.literal(t.name()), panelLeft + 32, y, 0xFFFFFFFF, true);

            int pos = att.techniques().positionOf(i);
            Component sub = Component.translatable(t.type().nameKey())
                    .append(Component.literal(" · " + t.size()
                            + (pos >= 0 ? " · [" + (pos + 1) + "]" : "")));
            g.drawString(this.font, sub, panelLeft + 32, y + 9, 0xFFAAAAAA, true);
            y += ROW_H;
        }
    }
