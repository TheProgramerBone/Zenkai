...
import com.hmc.zenkai.mixin.client.CameraAccessor;
 ...
 * rearmaría completo y bastaría machacar la tecla para tener un martilleo constante.
 */
...
public final class SenseKiWarnings {
    ...
    private static final int SHAKE_TICKS = 20;      // ~1 s
    private static final double SHAKE_AMP = 0.22;   // bloques, a intensidad máxima
    ...
    private static final class Track {
        long lastPl;
        long lastSeen;
        long lastSpike = Long.MIN_VALUE;
    }

    private static final Map<Integer, Track> TRACKS = new HashMap<>();
    private static long lastAnyWarn = Long.MIN_VALUE;

    // Sacudida en curso.
    private static long shakeStart = Long.MIN_VALUE;
    private static float shakeIntensity = 0f;

    /** Olvida completo. Cambio de mundo o desconexión: los ids de entidad ya no significan nada. */
    public static void forget() {
        TRACKS.clear();
        lastAnyWarn = Long.MIN_VALUE;
        shakeStart = Long.MIN_VALUE;
        shakeIntensity = 0f;
    }
    ...
    public static void onScan(Minecraft mc, List<SenseKiDataPacket.Entry> entries) {
        ...
        // TU PL REAL: te juzgas por lo que eres, no por lo que aparentas. Si usaras el aparente,
        // esconder tu propio ki haría que medio mundo pasara a ser una amenaza.
    ...
    /**
     * Desplazamiento puro que decae en ~1 s. Sin rotación: girarte la cámara te quita la
     * puntería y marea, y esto debe alarmar, no castigar.
     * Las tres frecuencias son deliberadamente inconmensurables entre sí: con frecuencias en
     * proporción simple el patrón se repite y se lee como una vibración mecánica en vez de
     * como un escalofrío.
     */
    @SubscribeEvent
    public static void onCamera(ViewportEvent.ComputeCameraAngles e) {
        if (shakeStart == Long.MIN_VALUE) return;

        Minecraft mc = Minecraft.getInstance();
        if (mc.level == null) { shakeStart = Long.MIN_VALUE; return; }

        float pt = (float) e.getPartialTick();
        float elapsed = (mc.level.getGameTime() + pt) - shakeStart;
        if (elapsed < 0f || elapsed >= SHAKE_TICKS) {
            shakeStart = Long.MIN_VALUE;
            return;
        }
        float decay = 1f - (elapsed / SHAKE_TICKS);
        float amp = (float) (SHAKE_AMP * shakeIntensity * decay * decay);

        float side = amp * Mth.sin(elapsed * 2.31f);
        float up   = amp * Mth.sin(elapsed * 3.07f + 1.7f) * 0.8f;
        float fwd  = amp * Mth.sin(elapsed * 1.63f + 3.1f) * 0.4f;

        ((CameraAccessor) e.getCamera()).zenkai$move(fwd, up, side);
    }
}